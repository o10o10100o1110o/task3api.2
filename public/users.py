...
users_list = [Main_UserDB(name='Ivanov', id=52, password='*******'),
              Main_UserDB(name='Petrov', id=54, password='********')]
...
def get_user(id: int):
    ...
    print(user)
    if user == None:
        return New_Respons(messages="Пользователь не найден")
...
def edit_person(item: Annotated[Main_User, Body(embed=True, description="Замена данных для пользователя по id")]):
